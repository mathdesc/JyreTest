...
public class ZreMsg 
{
    ...
    public static ZreMsg recv (Socket input)
    {
        ...
        try {
            ...
            //  Error returns
            Log.e("ZreMsg", "malformed message " + self.id);
            self.destroy ();
            return null;
        } finally {
            if (frame != null)
    ...
    public boolean send (Socket socket)
    {
        ...
        default:
            Log.e("ZreMsg", "bad message type  " + id+ ", not send");
            assert (false);
        }
...
//  Send the PING_OK to the socket in one step
    ...
    {
        Log.d("ZreMsg","     "+ entry.getKey ()+"="+entry.getValue ());
    }
    ...
    public void dump ()
    {
        ...
        case HELLO:
            ...
            Log.d("ZreMsg", "   sequence=" + sequence);
            if (ipaddress != null)
                Log.d("ZreMsg", "   ipaddress="+ipaddress);
            else
                Log.d("ZreMsg", "   ipaddress=''");
            Log.d("ZreMsg", "   mailbox="+mailbox);
            Log.d("ZreMsg", "   groups={");
            if (groups != null) {
                for (String value : groups) {
                    Log.d("ZreMsg"," '"+value+"'");
            ...
            Log.d("ZreMsg", "   }");
            Log.d("ZreMsg", "   status=" + status);
            Log.d("ZreMsg", "   headers={");
            if (headers != null) {
                for (Map.Entry <String, String> entry : headers.entrySet ())
        ...
        case WHISPER:
            Log.d("ZreMsg", "WHISPER:");
            Log.d("ZreMsg", "   sequence=" + sequence);
            Log.d("ZreMsg", "   content={");
            if (content != null) {
                int size = content.size ();
                byte [] data = content.getData ();
                Log.d("ZreMsg", "        size="+ content.size());
                if (size > 32)
                    size = 32;
                int contentIndex;
                for (contentIndex = 0; contentIndex < size; contentIndex++) {
                    if (contentIndex != 0 && (contentIndex % 4 == 0))
                        System.out.printf ("-");
                    Log.d("ZreMsg", " "+data [contentIndex]);
                }
            }
            Log.d("ZreMsg", "   }");
            break;
